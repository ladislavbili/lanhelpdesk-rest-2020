export const scheduledFilterSQL = (from, to, userId) => {
  ...
  if (from) {
    ...
    where.push(`(
      ("ScheduledTask"."from" >= '${from.toISOString().slice(0, 19).replace('T', ' ')}') OR
      ("ScheduledTask"."to" >= '${from.toISOString().slice(0, 19).replace('T', ' ')}')
    )`)
  }
  if (to) {
    to = new Date(to);
    ...
    where.push(`(
      ("ScheduledTask"."from" <= '${to.toISOString().slice(0, 19).replace('T', ' ')}') OR
      ("ScheduledTask"."to" <= '${to.toISOString().slice(0, 19).replace('T', ' ')}')
    )`)
  };
